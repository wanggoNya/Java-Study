		...
		try {
			...
			while (rs.next()) {
				products.add(new ProductDTO(rs.getString("pid"), rs.getString("pname"), rs.getString("pprice"),
						rs.getString("pdesc"), rs.getString("pmanu"), rs.getString("pcate"), rs.getString("pcondi"), rs.getString("pimage")));
			}
			return products;
...
public ProductDTO getDetail(String pid) throws NamingException, SQLException{
	...
	try {
		...
		ProductDTO product = new ProductDTO(id, pname, pprice, pdesc, pmanu, pcate, pcondi, pimage); // 객체 생성
