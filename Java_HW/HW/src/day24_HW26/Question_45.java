...
class DfsMap {
	...
	public static int min = n * m; // 경로 최소값 설정 (최대로 가능한 경로수만큼 최초 설정)
	public static int goal_x = n-1, goal_y = m-1; // 목표 지점의 x, y 좌표
	public static int search_count = 0; // 미로 탐색 수
	public static int[][] map = new int[1000][1000]; // 미로 사이즈 생성

	// DFS 탐색
	public void dfsRootSearch(int x, int y, int r) {
		// 목표 지점에 도달했다면
...
public class Question_45 {
	public static void main(String[] args) {
		DfsMap dfsmap = new DfsMap();
		// map 입력받기
		Scanner sc = new Scanner(System.in);
		System.out.print("N을 입력하세요 (4<=N, M<=200 : ");
		dfsmap.n = sc.nextInt(); // N, M을 공백을 기준으로 구분하여 입력 받기
		System.out.print("M을 입력하세요 (4<=N, M<=200 : ");
		dfsmap.m = sc.nextInt();
		sc.nextLine(); // 버퍼 지우기
		
		for (int i = 0; i < dfsmap.n; i++) { // 2차원 리스트의 맵 정보 입력 받기
			String str = sc.nextLine();
			for (int j = 0; j < dfsmap.m; j++) {
				dfsmap.map[i][j] = str.charAt(j) - '0';
		...
		dfsmap.dfsRootSearch(0, 0, 1);
		// 최단 경로 출력
		System.out.println("\n\n미로의 최단 경로값 : " + dfsmap.min); // 정답 출력
	}
}
