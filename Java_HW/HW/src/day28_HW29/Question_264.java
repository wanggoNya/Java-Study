...
public class Question_264 {
	public static void main(String[] args) {
		...
		// 이것을 2, 3, 5 각각 for문을 돌려 어글리 넘버를 구했다.
		// TreeSet의 크기는 n보다 크면 된다.
		// 어차피 지수^3이므로 for문의 조건은 넉넉히 해도 된다.
		for(int i = 0 ; i < n/2 ; i++) {
			for(int j = 0 ; j < n/2 ; j++) {
				for(int k = 0 ; k < n/2 ; k++) {
					aglySet.add((int)Math.pow(2, i) // 2^0부터 n/2승
		...
		int index = 1;
		for (Integer k : aglySet) { // for문을 돌려서 index에 걸리는 숫자 출력을 위한 for문 구한다.
			if (index == n) {
